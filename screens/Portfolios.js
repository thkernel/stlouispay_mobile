...
_showPortfoliosDetails = async (portfolio) => {
    let {navigation} = this.props;

    navigation.navigate("ShowPortfoliosDetails",{
      portfolio: porfolio
    })
  }
  ...
  _renderPortfolios() {
        if (this.props.portfolios != null && this.props.portfolios != undefined && this.props.portfolios.length > 0) {
          
          return (
            this.props.portfolios.map( (portfolioItem,index) => {
                return(
                   <ListItem avatar key={index} onPress={()=> {                
                     this._showPortfolioDetails(portfolioItem);
                     console.log("PORTFOLIO ITEM WAS PRESSED: ", portfolioItem);
                   }}>

                      <Left>
                      <Thumbnail source={{uri: portfolioItem.cover_url}}   style={styles._h_w_avatar} />
                      </Left>

                      <Body>
                        <Text numberOfLines={1} style={styles._h_w_names}>{portfolioItem.title}</Text>
                      </Body>
                      <Right>
          ...
          const height =
            ...
            if(this.props.loadingPortfolios){
              return (
                <View style={{ flex: 1, alignItems: "center", marginTop: height }}>
                  <ActivityIndicator size="large"/>
                </View>
              );
            }else if(this.props.portfoliosEmpty){
              return (
